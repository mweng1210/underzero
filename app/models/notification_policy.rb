...
class NotificationPolicy < ApplicationRecord
  ...
  def pending_notification_requests
    @pending_notification_requests ||= notification_requests.without_unavailable.limit(MAX_MEANINGFUL_COUNT).pick(Arel.sql('count(*), coalesce(sum(notifications_count), 0)::bigint'))
  end
end
