...
class Api::V1::Notifications::RequestsController < Api::BaseController
  ...
  def load_requests
    requests = NotificationRequest.where(account: current_account).without_unavailable.includes(:last_status, from_account: [:account_stat, :user]).to_a_paginated_by_id(
